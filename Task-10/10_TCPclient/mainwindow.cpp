...
{
    ui->tb_result->append("Текущее время и дата на сервере: " + time.time().toString() + ", " + time.date().toString());
}
...
{
    ui->tb_result->append("На сервер отправлена строка: " + replyString);
}
...
void MainWindow::DisplayStat(StatServer stat)
{
    ...
    ui->tb_result->append("Передано байт: " + QString::number(stat.revPck) + " байт");
...
void MainWindow::DisplayError(uint16_t error)
{
    ...
    case ERR_NO_FUNCT:
        ...
        msg.setText("Такой запрос не реализован в текущей версии!");
        msg.exec();
...
void MainWindow::on_pb_request_clicked()
{
   ...
   header.len = 0;
    ...
    switch (ui->cb_request->currentIndex()){
        ...
        default:
            emit client->sig_Error(ERR_NO_FUNCT);
            return;
   }
