...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        resultRace1 = QtConcurrent::run(&ExampleRace::DoWork, concurRace1, &number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());
        while(!resultRace1.isFinished()) {continue;}

        resultRace2 = QtConcurrent::run(&ExampleRace::DoWork, concurRace2, &number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());
        while(!resultRace2.isFinished()) {continue;}
